...
エッカートは7歳の頃には塹壕ラジオを作っていたように、若い頃から様々な音を用いるテクノロジーへの興味を持っていた。高校生の頃には礼拝堂で鐘の音を流すためのサウンドシステムを作って金をもらっていたりもしていた。それだけでなく、嘘発見機の仕組みをベースにして作られた「osculometer(キス測定装置)」のような、結構アホみたいな装置も沢山作っていたそうである。今日であれば、MakerFaireとかに出展されててもおかしくはない、ある種のエンターテインメント目的の装置である。
...
Civil（市民）、とあるが、英語のCivil Engineeringとは概ね軍事技術でない工学、という意味合いを込めて使われることが多い。この由来はいくつかあるらしいのだが、1750 年ごろにイギリスの工学者ジョン・スミートンが自らの専門を、相容れない軍事的な 研究と区別するために名乗り始めたことがひとつの起源とされている[^florman]。

[^florman]: Florman, Samuel C. _The Civilized Engineer_. St. Martin’s Griffin, 1989.

電子計算機ほど、軍事的な用途に由来しつつ、Civilizedされた技術も他にないのかもしれない——などと書こうとと思ったが、考えてみると無線/優先、電気/音響問わず、あらゆる種類の通信技術、メディア技術ほどWW2以降に軍事的に活用された技術も他にない。そして、通信技術がそのように発展したからこそ、多くのコンピューターに関わる技術はベル研究所やRCAのような電話やラジオの技術研究の場を中心に発展してきた。
...
音楽のためのプログラミング環境としてもっとも有名もののMaxとPureDataというシステムには`bang`というオブジェクト（処理の基礎単位）が存在する。`bang`というのはデータの種類（型）をあらわすものでもあって、普通のプログラミング言語でいうと`void`や`unit`などと呼ばれる、データの中身を持たない型のことを指す。
...
ところで、MaxとPureDataのオリジナルの開発者であり、今でもPureDataの開発を続けているミラー・パケットは、この`bang`や、同様に基礎的なオブジェクトである`trigger`という語の由来は半ばノリでの命名であったものの、当時のコンピューター音楽と軍事技術との接点があったことを示唆している。[^puckette]
...
60~70年代、冷戦を背景にした初期のコンピューター音楽に関わる研究の多くが軍事的な予算の元に開発された。有名なものでは、ジョン・チョウニングが開発したFM合成の研究なんかは全力でARPA（現在のDARPA）の予算の元開発されていた[^chowning]。このFM合成の発見には、Maxの大祖先にあたらうMUSIC IVという音楽プログラミング環境が利用されていた。
...
それに、軍事的な予算への依拠だけが音楽コンピューティングと軍事技術の関わりではなかった。
...
1994年の季刊インターコミュニケーション9号 特集：音=楽テクノロジーには、ブーレーズに関する論考の中で4Xに触れている記事が2つ隣あっている[^intercomm]。1つは作曲家野平一郎の記事。もう1つは美術史家のオットー・カール・ヴェルクマイスターの記事である。
